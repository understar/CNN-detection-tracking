...
因为local Conv的原因会导致转换失败；难道需要重新设置kit.cfg???
目前支持Layertype：
...
class DecafNet(object):
    ...
    def __init__(self, net_file=None, meta_file=None):
        ...
        self._net = translator.translate_cuda_network(
            cuda_decafnet, {'data': (INPUT_DIM, INPUT_DIM, 1)})
    ...
    def classify_direct(self, images):
        ...
        Input:
            images: a numpy array of size (num x 40 x 40 x 1), dtype
                float32, c_contiguous, and has the mean subtracted and the
                image flipped if necessary.
        Output:
            scores: a numpy array of size (num x 2) containing the
    ...
    def oversample(image, center_only=False):
        ...
        Input:
            image: an image of size (40 x 40 x 1) and has data type uint8.
